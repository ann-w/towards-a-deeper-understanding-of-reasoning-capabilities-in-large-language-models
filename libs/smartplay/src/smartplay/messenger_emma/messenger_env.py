...
def describe_frame(info):
    ...
    for idx in np.unique(info):
        ...
        smallest = np.unravel_index(
            np.argmin(np.where(info == idx, dist, np.inf)), info.shape
        )
        obj_info_list.append(
            (
                describe_block(idx),
                dist[smallest[:2]][0],
                describe_loc(np.array([0, 0]), smallest[:2] - center),
            )
        )
            ...
            "\n".join(
                [
                    (
                        ...
                        if dist > 0
                        else "- {} 0 steps with you".format(name)
                    )
...
class MessengerEnv(gym.Env):
    ...
    def use_shaping(self):
        return self._env.use_shaping

    @property
    def use_text_substitution(self):
        return self._env.use_text_substitution

    default_iter = 20
    default_steps = None

    # def __init__(self, lvl: int =1, max_steps: int = 2, env_noise: int = 0):

    def __init__(self, lvl: int = 1, max_steps: int = 2, env_noise: int = 0, **kwargs):

        super().__init__()
        self.lvl = lvl

        # assign correct number of steps to level
        # lvl_to_steps = [4, 64, 128]
        lvl_to_steps = [10, 64, 128]
