...
from messenger.envs.manual import Descr, TextManual
...
class StageThree(MessengerEnv):
    ...
    def _convert_obs(self, vgdl_obs):
        ...
        if "decoy_message.1" in vgdl_obs:
            entity_locs.add(
                self.game.message, Position(*vgdl_obs["decoy_message.1"]["position"])
            )
        if "decoy_goal.1" in vgdl_obs:
            entity_locs.add(
                self.game.goal, Position(*vgdl_obs["decoy_goal.1"]["position"])
            )
    ...
    def reset(self, variant_id: int = None, **kwargs):
        ...
        self.game = random.choice(
            self.all_games
        )  # (e.g. enemy-alien, message-knight, goal - bear)
        ...
        all_npcs = (
            ...
            Descr(
                entity=self.game.message.name, role="message", type=variant.message_type
            ),
            Descr(entity=self.game.goal.name, role="goal", type=variant.goal_type),
            Descr(
                entity=self.game.message.name,
                role="enemy",
                type=variant.decoy_message_type,
            ),
            Descr(
                entity=self.game.goal.name, role="enemy", type=variant.decoy_goal_type
            ),
        )
