...
def describe_env(info):
    ...
    obs = "You face {} at your front.".format(
        target, describe_loc(np.array([0, 0]), facing)
    )
...
class Crafter(Env):
    ...
    def __init__(
        ...
        with open(
            root / "assets/crafter_ctxt.pkl", "rb"
        ) as f:  # Context extracted using text-davinci-003 following https://arxiv.org/abs/2305.15486
            CTXT = pickle.load(f)
        CTXT = CTXT.replace("DO NOT answer in LaTeX.", "")
        CTXT = CTXT.replace(
            "Move Up: Flat ground above the agent.",
            "Move North: Flat ground north of the agent.",
        )
        CTXT = CTXT.replace(
            "Move Down: Flat ground below the agent.",
            "Move South: Flat ground south of the agent.",
        )
        CTXT = CTXT.replace(
            "Move Left: Flat ground left to the agent.",
            "Move West: Flat ground west of the agent.",
        )
        CTXT = CTXT.replace(
            "Move Right: Flat ground right to the agent.",
            "Move East: Flat ground east of the agent.",
        )
        self.desc = CTXT
    ...
    def step(self, action):
        ...
        self.score_tracker = self.score_tracker + sum(
            [1.0 for k, v in info["achievements"].items() if v > 0]
        )
        info.update(
            {
