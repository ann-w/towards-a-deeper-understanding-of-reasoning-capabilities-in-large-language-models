...
class BanditEnv(gym.Env):
    ...
    def __init__(self, p_dist, r_dist, max_steps=50):
        ...
        self.action_list = [
            "Pull slot machine {}.".format(i + 1) for i in range(self.n_bandits)
        ]
        ...
        if np.random.uniform() < self.p_dist[action]:
            if not isinstance(self.r_dist[action], list):
                ...
                reward = np.random.normal(
                    self.r_dist[action][0], self.r_dist[action][1]
                )

        self.score_tracker += int(action == self.optimal)
        info = {
            "obs": "You pulled slot machine {}, you received reward {}.".format(
                action + 1, reward
            ),
