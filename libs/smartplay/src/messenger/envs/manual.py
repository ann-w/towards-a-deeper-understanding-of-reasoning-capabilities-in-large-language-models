...
class TextManual:
    ...
    def get_document(
        ...
        **kwargs
        ...
        document = [
            self.get_descriptor(
                entity=enemy, entity_type=enemy_type, role="enemy", **kwargs
            ),
            self.get_descriptor(
                entity=message, entity_type=message_type, role="message", **kwargs
            ),
            self.get_descriptor(
                entity=goal, entity_type=goal_type, role="goal", **kwargs
            ),
        ]
        ...
        if append:
            ...
            valid_objs = [
                obj.name
                for obj in config.NPCS
                if obj.name not in [enemy, message, goal]
            ]
