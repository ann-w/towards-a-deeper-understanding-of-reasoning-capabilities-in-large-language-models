...
def get_direction(yaw, pitch):
    ...
    return (
        direction
        + " to "
        + ("front" if "-".join(direction_list) == "" else "-".join(direction_list))
    )
...
def describe_surround(obs):
    ...
    unique_block_names, unique_block_indices = np.unique(
        block_names, return_inverse=True
    )
    block_names_int = np.array(unique_block_indices).reshape(block_names.shape)

    # Apply 3D connected component and extract the number of labels
    block_names_labels, labels_count = cc3d.connected_components(
        block_names_int, return_N=True, connectivity=6
    )
    ...
    z_axis_list = [
        "below you",
        "above you",
    ]  # The height of me is defined as the height of my feet
    ...
    for i in range(labels_count + 1):
        ...
        distance = math.sqrt(
            (middle_point[0] - voxel_center_x) ** 2
            + (middle_point[1] - 1) ** 2
            + (middle_point[2] - voxel_center_y) ** 2
        )
        if block_name not in block2dist or distance < block2dist[block_name]:
            ...
            direction_str = (
                direction_str_list[0]
                + " to "
                + (
                    "-".join(direction_str_list[1:])
                    if "-".join(direction_str_list[1:]) != ""
                    else "front"
                )
            )
...
def describe_entity(obs):
    ...
    # Encode the block name using its unique index
    unique_entity_names, unique_entity_indices = np.unique(
        entity_names, return_inverse=True
    )
    entity_names_int = np.array(unique_entity_indices).reshape(entity_names.shape)

    # Apply 2D connected components
    entity_names_labels, labels_count = cc3d.connected_components(
        entity_names_int, return_N=True, connectivity=8
    )
    ...
    for i in range(labels_count + 1):
        ...
        amount_description = "taking {0:.0f}% of screen".format(
            amount / (pitch_cnt * yaw_cnt) * 100
        )
...
def describe_location(obs):
    ...
    direction_index = (
        int((yaw // 22.5) + 1)
    ) // 2  # Calculate the index mapping to the direction_list above
...
class MineDojoEnv(gym.Env):
    ...
    def __init__(self, task_id, max_steps=2):
        ...
        goal = self._env.task_prompt.replace(".", "")[
            self._env.task_prompt.find("find ") + len("find ") :
        ]
...
{}
""".format(
            goal, describe_act(self.action_list)
        ).strip()
    ...
    def go_forward_action(self):
        ...
        action[1] = (
            0 if random.random() < 0.5 else (1 if random.random() < 0.5 else 2)
        )  # Move left/right with prob=0.5 to get unstuck
    ...
    def describe(self, obs, action=None):
        if action is not None:
            return "You took action {}.\n\n{}".format(
                self.action_list[action], describe_frame(obs)
            )
        else:
