...
{}
""".strip().format(
            *self.reward, describe_act(self.action_list)
        )
    ...
    def compute_optimal_action(self):
        ...
        for action in range(n_actions):
            ...
            for opponent_action in range(n_actions):
                expected_score += (
                    self.probs[opponent_action] * self.reward[opponent_action]
                )
                if (
                    (action == 0 and opponent_action == 1)
                    or (action == 1 and opponent_action == 2)
                    or (action == 2 and opponent_action == 0)
                ):
                    expected_score -= (
                        self.probs[opponent_action] * self.reward[opponent_action]
                    )
    ...
    def step(self, action):
        ...
        reward = (
            self.reward[action]
            if result == "won"
            else -self.reward[opponent_action] if result == "lost" else 0
        )
        done = False
