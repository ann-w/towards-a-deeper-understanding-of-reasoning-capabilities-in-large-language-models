...
parser.add_argument(
    "--env_names",
    type=str,
    default="BanditTwoArmedHighLowFixed",
...
def compose_ingame_prompt(info, question, past_qa=[]):
    ...
    messages.append(
        {
            "role": "system",
            "content": "current step observation: {}".format(info["obs"]),
        }
    )
    ...
    return (
        messages,
        2,
    )  # This is the index of the history, we will truncate the history if it is too long for LLM
...
def run(env_name):
    ...
    def match_act(output):
        inds = [
            (i, output.lower().index(act.lower()))
            for i, act in enumerate(env.action_list)
            if act.lower() in output.lower()
        ]
        if len(inds) > 0:
    ...
    progresses = []
    for eps in tqdm(
        range(num_iter), desc="Evaluating LLM {} on {}".format(LLM_name, env_name)
    ):
        import wandb
        ...
        columns = (
            ["Context", "Step", "OBS", "History", "Score", "Reward", "Total Reward"]
            + questions
            + ["Action"]
        )
        ...
        while step < env_steps:
            ...
            wandb.log(
                {
                    "metric/total_reward".format(eps): sum(rewards),
        ...
        wandb.log(
            {
                "rollout/rollout".format(eps): wandb_table,
